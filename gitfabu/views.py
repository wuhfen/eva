...
def conf_add(request, env):
    ...
    if request.method == 'POST':
        ...
        if platform == "现金网":  # 注意：只有现金网和蛮牛的web项目发布时才会创建git地址，其他的单独项目和JAVA项目应该手动添加git的相关信息
            ...
            js_pc, js_pc_repo = git_coderepo.objects.get_or_create(platform=platform, classify=envir, title=name+"_js_pc", defaults={'address': online_git+"ds-vue-group/pc/"+name+".git", 'user': username, 'passwd': password})
            js_mob, js_mob_repo = git_coderepo.objects.get_or_create(platform=platform, classify=envir, title=name+"_js_mobile", defaults={'address':online_git+"ds-vue-group/m/"+name+".git", 'user': username, 'passwd': password})
